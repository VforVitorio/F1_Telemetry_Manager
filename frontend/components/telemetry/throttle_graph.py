...
def render_throttle_graph(telemetry_data_multi, selected_drivers, color_palette):
    """
    ...
    if telemetry_data_multi is not None and isinstance(telemetry_data_multi, dict) and telemetry_data_multi:
        ...
        for idx, driver in enumerate(selected_drivers):
            if driver in telemetry_data_multi:
                ...
                if 'distance' in driver_telemetry and 'throttle' in driver_telemetry:
                    distance = driver_telemetry.get('distance', [])
                    throttle = driver_telemetry.get('throttle', [])
        ...
        if df_list:
            ...
            _render_section_title_with_button(fig, drivers_with_data[0], "throttle")
        ...
        if not driver_data.empty:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode='lines',
                    hovertemplate='<b>%{fullData.name}</b><br>Distance: %{x:.0f}m<br>Throttle: %{y:.1f}%<extra></extra>'
                )
            )

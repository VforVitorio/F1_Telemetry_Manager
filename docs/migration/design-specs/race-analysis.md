...
- `POST /api/v1/strategy/rag {question, year?}` (`:656`, burst 5) → `result: {question, answer, chunks[{text, article, doc_type, year, score}], articles[]}`. Omit `year` for the historical unscoped lookup; provide it to select one FIA rulebook. Cite from `articles`, never from answer prose (`rag_agent.py:84-88`: the LLM may hallucinate article numbers; chunk metadata is reliable).

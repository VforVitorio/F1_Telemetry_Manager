...
heavy strategy / telemetry import surface out: those routers import
...
about: **a provider failure must not surface as HTTP 200.** Both ``/message``
and ``/tool-message`` map a provider failure to 503 (pinned below).
"""
